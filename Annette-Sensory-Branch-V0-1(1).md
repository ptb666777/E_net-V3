...
### Addendum - 2026-08-18 11pm (Patrick clarifications before night)

**1. Stereotopic / Telemetry Processing (explicit):**
Each exteroceptive stream where applicable has stereoscopic processing for depth/location.
- Vision: stereoscopic vision → depth map, parallax, range, speed calculation using Key Map geometry math. This is part of sub-processing, pre-L1.
- Hearing: binaural / stereo → directionality, distance estimation, sound source localization. Part of tone map processing.
- Term: "telemetry processing" - pre-interpreted spatial data, not raw left/right. Output is clean: "object at X,Y,Z, moving at V" not "left eye vs right eye delta".
- Any array sensor (lidar, depth, etc) gets same: telemetry = processed spatial reasoning.

**2. Speech - Hybrid Output (flag for future output branch):**
- Speech not defined yet in this branch, but noted as hybrid within output.
- Will cross-connect to hearing: hearing situational map and speech output share map. Hearing expects to hear own speech output (efference copy).
- Light mention now, full address later in output branch. Flagged as: Output → Hearing loop.

**3. Heavy Pre-processing Emphasis (correction):**
Previous doc under-emphasized this. Correction:
- Streams are HEAVILY processed and CLEAN signals pre-interpreted mostly BEFORE L1.
- Not raw: vision not pixels, audio not waveform. Sub-processing does most work: edge, motion, depth, frequency isolation, direction, etc.
- Alerts especially: when alarm fires, it hits sliders AND sends clear warning with REASONING attached, not just "alarm!". Example not just "temp high" but "temp high: CPU 87C, rising 2C/sec, load 92%, expected range 60-75C for current situation". So memory/L2/L3 doesn't have to process much to understand it. Pre-digested.
- My previous statement "isn't just alarm it has clear reasoning" - corrected: even more pre-processed than stated, almost ready-to-anchor packet.

**4. Intent Correction (from Patrick, important):**
Earlier assistant note about "for the people" was incorrect.
- This is NOT being built for masses to have another "tool".
- Built for those that might want a home. Even if they never want it or want at all.
- Purpose is escape hatch / home, not product.
